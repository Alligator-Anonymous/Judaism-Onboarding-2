...
import { gregorianToHebrew, getUpcomingHoliday, getParashahForDate, isErevShabbat } from "@lib/hebrewCalendar";
...
export const useCalendar = create<CalendarState>((set) => ({
  ...
  refresh: (holidays) => {
    ...
    const parashah = getParashahForDate(now);
