...
export const FAQ: React.FC = () => {
  const entries = Object.values(useContent((state) => state.registry?.faq ?? {}));
    ...
    <div className="space-y-4">
      {entries.map((entry) => (
        <article key={entry.id} className="rounded-xl border border-slate-200 p-4 shadow-sm dark:border-slate-700">
          ...
          {entry.disclaimer ? <p className="mt-2 text-xs text-slate-500">{entry.disclaimer}</p> : null}
          {entry.license ? <p className="text-[0.65rem] text-slate-400">License: {entry.license}</p> : null}
        </article>
      ))}
      {entries.length === 0 ? <p className="text-sm text-slate-500">FAQ content is loading…</p> : null}
    </div>
  );
};
